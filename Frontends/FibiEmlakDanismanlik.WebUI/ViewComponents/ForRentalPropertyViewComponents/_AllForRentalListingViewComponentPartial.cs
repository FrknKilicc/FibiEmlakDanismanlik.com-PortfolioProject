...
{
    public class _AllForRentalListingViewComponentPartial:ViewComponent
    {
       
            private readonly IConfiguration _configuration;
            private readonly IHttpClientFactory _httpClientFactory;
            public _AllForRentalListingViewComponentPartial(IConfiguration configuration, IHttpClientFactory httpClientFactory)
            {
            ...
            public async Task<IViewComponentResult> InvokeAsync()
            {
                ...
                // 3) facets
                var facetResp = await client.GetAsync($"{apiUrl}ForRental/GetForRentalListingTypeFacets");
                ...
                var listResp = await client.PostAsync($"{apiBase}ForRental/ForRentalFilter", body);
                var items = new List<RentalListingBaseDto>();
                var total = 0;
                ...
                var respObj = JsonConvert.DeserializeObject<RentalFilterResponseDto>(json);
                if (respObj != null)
                {
                ...
                var vm = new ForRentalListingPageVm
                {
